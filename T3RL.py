...
def train_eval_t3(args):
    ...
    num_parallel_envs = 1
    num_train_epochs = 25
    num_iterations = 100000 if train else 0
    ...
    with eager_mode():
        ...
        if train:
            ...
            dataset = replay_buffer.as_dataset(
                sample_batch_size=train_batch_size, num_steps=2
            ).prefetch(3)
            ...
            for _ in range(num_iterations):
                ...
                if step % checkpoint_interval == 0:
                    ...
                    saved_model_path = os.path.join(
                        saved_model_dir, "policy_" + (f"{step}").zfill(9)
                    )
    ...
    def evaluate_human(eval_env, eval_policy):
        time_step = eval_env.reset()
        count = 0
        while not time_step.is_last():
            if count % 2 == 0:
                action = int(input("Enter you move: (0 - 8): "))
            else:
                action = eval_policy.action(time_step)
            time_step = eval_env.step(action)
            count += 1
    if eval_:    
        evaluate_human(eval_env, eval_policy)


def parse_args():

    parser = argparse.ArgumentParser(
        description="A RL agent for classical tic-tac-toe game.", add_help=True
    )
